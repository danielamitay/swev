...
/// NFC byte-level BPE configured by the embedded tokenizer document.
...
struct ByteBPETokenizer {
    ...
    init(data: Data) throws {
        ...
        for key in ["dropout", "unk_token"] {
            guard model[key] == nil || model[key] is NSNull else { throw SwevError.unsupportedTokenizer }
        }
        ...
        for (rank, merge) in merges.enumerated() {
            ...
            guard parts.count == 2, vocabulary[parts[0] + parts[1]] != nil,
                  ranks.updateValue(rank, forKey: Pair(left: parts[0], right: parts[1])) == nil else {
        ...
        self.split = try NSRegularExpression(pattern: pattern)
        ...
        guard mapping.enumerated().allSatisfy({ [192, 193].contains($0.offset) || $0.offset >= 245 || vocabulary[$0.element] != nil }) else { throw SwevError.unsupportedTokenizer }
        self.bytes = mapping
    }

    func tokenID(_ text: String) throws -> Int {
        guard let id = addedByText[text]?.id ?? vocabulary[text] else { throw SwevError.unsupportedTokenizer }
        return id
    }
    ...
    func encode(_ text: String) throws -> [Int] {
        ...
        return try extract(text, regex: rawAdded) { raw in
            try extract(raw.precomposedStringWithCanonicalMapping, regex: normalizedAdded, plain: encodePlain)
        }
    }
    ...
    private func encodePlain(_ text: String) throws -> [Int] {
        ...
        var result: [Int] = []
        var offset = 0
        for match in split.matches(in: text, range: NSRange(location: 0, length: string.length)) {
            guard match.range.location == offset else { throw SwevError.unsupportedTokenizer }
            offset = NSMaxRange(match.range)
            var symbols = string.substring(with: match.range).utf8.map { bytes[Int($0)] }
            guard symbols.count <= 4096 else { throw SwevError.resourceLimit }
            while symbols.count > 1 {
                var best: (index: Int, rank: Int)?
                for i in 0..<(symbols.count - 1) {
                    if let rank = ranks[Pair(left: symbols[i], right: symbols[i + 1])], rank < (best?.rank ?? Int.max) {
                        best = (i, rank)
                    }
                }
            ...
            for symbol in symbols {
                guard let id = vocabulary[symbol] else { throw SwevError.unsupportedTokenizer }
                result.append(id)
            }
        }
