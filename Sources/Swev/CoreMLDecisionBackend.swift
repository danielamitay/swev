...
struct CoreMLModelDescriptor: Decodable, Sendable {
    ...
    static func read(metadata: [String: String]) throws -> CoreMLModelDescriptor {
...
actor CoreMLDecisionBackend {
    public nonisolated let descriptor: CoreMLModelDescriptor
    ...
    public static func load(from modelURL: URL, configuration: RuntimeConfiguration = .init()) async throws -> CoreMLDecisionBackend {
        try Task.checkCancellation()
        ...
        let task = Task.detached { try await CoreMLDecisionBackend(contentsOf: modelURL, configuration: configuration) }
