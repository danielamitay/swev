...
@Test func decisionContentCompletesChannelHeadersWithoutGenerating() throws {
    ...
    let continuation = AssistantContinuation(encode: encode, decode: decode)
    ...
    let plain = AssistantContinuation(encode: { $0.utf8.map(Int.init) }, decode: { String(bytes: $0.map(UInt8.init), encoding: .utf8)! })
